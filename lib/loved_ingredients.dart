...
class LovedIngredientsScreen extends StatefulWidget {
  final List<String> ingredients;

  const LovedIngredientsScreen({super.key, required this.ingredients});

  @override
  _LovedIngredientsScreenState createState() => _LovedIngredientsScreenState();
}

class _LovedIngredientsScreenState extends State<LovedIngredientsScreen> {
  ...
  Future<void> _loadSelectedIngredients() async {
    ...
    final response = await http
        .get(Uri.parse('api-link/user/$userId/selectedLovedIngredients'));
    ...
    if (response.statusCode == 200) {
      ...
      // Handle error
      print('Failed to load loved ingredients');
    }
  }
  ...
  Future<void> _saveSelectedIngredients() async {
    ...
    if (response.statusCode != 200) {
      // Handle error
      print('Failed to save loved ingredients');
    }
  }
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Loved Ingredients'),
          bottom: const TabBar(
            tabs: [
              Tab(text: 'Search'),
              Tab(text: 'Selected'),
